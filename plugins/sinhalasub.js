...
const NodeCache = require('node-cache'); // NodeCache library එක import කිරීම
...
// [FIXED] searchCache විචල්‍යය නිවැරදිව new keyword එක යොදා ආරම්භ කිරීම
...
cmd({
    ...
    try {
        ...
        // ඔබට N/A වෙනුවට සත්‍ය දත්ත පෙන්වීමට අවශ්‍ය නම්, 
        // ඔබ console log එකේ දුටු නිවැරදි names මෙහි යාවත්කාලීන කළ යුතුය.
        const searchResults = searchData.data.map((item, index) => ({
            n: index + 1,
            // (1) title සඳහා නිවැරදි නම යොදන්න:
            title: item.m_name || item.movie_title || item.name || item.title || 'N/A', 
            // (2) imdb සඳහා නිවැරදි නම යොදන්න:
            imdb: item.m_rating || item.imdb_rating || item.rating || item.imdb || 'N/A',      
            // (3) year සඳහා නිවැරදි නම යොදන්න:
